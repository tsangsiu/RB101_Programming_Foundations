...
Algorithm:
  ...
  - Initalise an empty string, say `longest_sentence` to store the longest
    sentence.
  - Initialise an empty array, say `sentences` to store the sentences
  - While the text contain '.', '!', '?'
    - find the index of their first occurrence
    - slice the string from the beginning to and including that position
    - append the sliced string to `sentences`
    - slice away the sliced text
  - For each sentence,
